...
;(function(window) {
	...
	*******/
	...
	function GridFx(el, options) {
		...
		this.theaterEl = nextSibling(this.previewEl);
		...
		this.theaterPlayerEl = this.theaterEl.querySelector('iframe');
		this._init();
	}
	...
	GridFx.prototype.options = {
		...
		onPlayerOpen : function(instance, item, player) { return false;},
	...
	GridFx.prototype._openItem = function(ev, item) {
		...
		var startPreview = this.previewDescriptionEl.querySelector('.preview-button');
		var clickEvent = (document.ontouchstart!==null ? 'click' : 'touchstart');
		
		startPreview.addEventListener(clickEvent, function(ev){
			if(clickEvent === 'click') {
				console.log('Open player.');
				//ev.preventDefault();
				self._openPlayer(ev, item);
			} else {
				//TODO: this is not implemented yet (No preview on mobile)
				alert('Preview not yet implemented on mobile.');
				//manageTouch();
			}
		});
	...
	GridFx.prototype._openPlayer = function(ev, item) {
		console.log('_openPlayer called.');
		if(this.isAnimating) return;
		console.log('Passed animating check.');
		this.isAnimating = true;
		this.isExpanded = true;
		...
		//Point the iframe to the right video
		var embedCodeEl = item.querySelector('.embed-code');
		if( embedCodeEl ) {
			var code = embedCodeEl.innerHTML;
			console.log('Embed code: ' + code + '.');
			
			//TODO: This is the iFrame method:
			//this.theaterPlayerEl.src = 'http://159.203.91.129/player/beta/PortolPlayer.html?embedCode='+code;
			
			//TODO: This is the popup method:
			var dest = 'http://159.203.91.129/player/alpha/PortolPlayer.html?embedCode='+code;
			var strWindowFeatures = "location=no,resizable=no,scrollbars=no,status=yes,width=1080,height=550,top=0,left=0";
			this.windowReference = window.open(dest, "Play", strWindowFeatures);
		} else {
			console.log('No embed code found');
		}
		
		var self = this;
		setTimeout(function() { 
			// controls the elements inside the expanded view
			classie.add(self.previewEl, 'is-removed');
			classie.add(self.theaterEl, 'vs-player-wrapper--open');
			// callback
			//self.options.onExpand();
		}, 0);
	};
	...
	GridFx.prototype._closeItem = function() {
		...
		onEndTransition(this.originalImg, function() {
			...
			//TODO: clear previewButton listener
	...
	/********
		Scrolling effects	
	 ********/
	GridFx.prototype._onScrollFn = function() {
			var self = this;
			if( !this.didScroll ) {
				this.didScroll = true;
	...
	GridFx.prototype._scrollPage = function() {
			var self = this;
			this.items.forEach( function( el, i ) {
				if( !classie.has( el, 'shown' ) && !classie.has( el, 'appear-animate' ) && inViewport( el, self.options.viewportFactor ) ) {
					setTimeout( function() {
						var perspY = scrollY() + getViewportH() / 2;
						self.gridEl.style.WebkitPerspectiveOrigin = '50% ' + perspY + 'px';
						self.gridEl.style.MozPerspectiveOrigin = '50% ' + perspY + 'px';
						self.gridEl.style.perspectiveOrigin = '50% ' + perspY + 'px';
	...
	GridFx.prototype._resizeHandler = function() {
			var self = this;
			function delayed() {
				self._scrollPage();
	...
	GridFx.prototype._checkTotalRendered = function() {
			this.itemsRenderedCount = this.itemsRenderedCount + 1;
