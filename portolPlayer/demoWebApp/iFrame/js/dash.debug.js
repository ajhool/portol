...
MediaPlayer = function(context) {
    ...
    var VERSION = "1.5.0", DEFAULT_TIME_SERVER = "http://time.akamai.com/?iso", DEFAULT_TIME_SOURCE_SCHEME = "urn:mpeg:dash:utc:http-xsdate:2014", numOfParallelRequestAllowed = 0, system, abrController, element, source, protectionController = null, protectionData = null, streamController, rulesController, playbackController, metricsExt, metricsModel, videoModel, DOMStorage, initialized = false, resetting = false, playing = false, autoPlay = true, scheduleWhilePaused = false, bufferMax = MediaPlayer.dependencies.BufferController.BUFFER_SIZE_REQUIRED, useManifestDateHeaderTimeSource = true, UTCTimingSources = [], liveDelayFragmentCount = 4, usePresentationDelay = false, mPortolData = null, isReady = function() {
        ...
        console.log(source);
        if (typeof source === "string" || source.isXML) {
    ...
    return {
        ...
        attachSource: function(urlOrManifest, protectionCtrl, pProtectionData, pPortolData) {
            if (!initialized) {
                throw "MediaPlayer not initialized!";
            }
            ...
            protectionData = pProtectionData;
            console.log("attachSource, about to get portolData");
...
MediaPlayer.dependencies.FragmentLoader = function() {
    "use strict";
    var RETRY_ATTEMPTS = 12, RETRY_INTERVAL = 500, xhrs = [], doLoad = function(request, remainingAttempts) {
...
MediaPlayer.dependencies.LiveEdgeFinder = function() {
    "use strict";
    var isSearchStarted = false, searchStartTime = NaN, rules, liveEdge = null, ruleSet = MediaPlayer.rules.SynchronizationRulesCollection.prototype.TIME_SYNCHRONIZED_RULES, onSearchCompleted = function(req) {
        ...
        if (e.error) {
            ...
            console.log("Using TIME_SYNCHRONIZED_RULES");
...
MediaPlayer.dependencies.ManifestLoader = function() {
    ...
    var RETRY_ATTEMPTS = 3, RETRY_INTERVAL = 500, parseBaseUrl = function(url) {
        ...
        return base;
    }, doXMLLoad = function(manifestXML) {
        var baseUrl = null, url = baseUrl, request = new XMLHttpRequest(), requestTime = new Date(), loadedTime = null, needFailureReport = true, manifest, xmlLoad, report, self = this;
        xmlLoad = function() {
            console.log("Calling manifestLoader.xmlLoad");
            needFailureReport = false;
            loadedTime = new Date();
            console.log("Starting to parse xml manifest @ ", loadedTime.getTime());
            self.metricsModel.addHttpRequest("stream", null, "MPD", url, null, null, requestTime, loadedTime, null, 200, null, null, null);
            manifest = self.parser.parse(manifestXML, null);
            if (manifest) {
                manifest.url = manifest.baseUrl;
                ...
                console.log("No manifest: ", manifest);
                self.notify(MediaPlayer.dependencies.ManifestLoader.eventList.ENAME_MANIFEST_LOADED, {
                    manifest: null
                }, new MediaPlayer.vo.Error(null, "Failed loading manifest: " + baseUrl, null));
            }
        };
        report = function() {
        ...
        this.log("Start loading XML manifest: ", manifestXML);
        xmlLoad();
    ...
    return {
        ...
        system: undefined,
        xlinkController: undefined,
        load: function(urlOrManifestXML) {
            if (urlOrManifestXML.isXML) {
                doXMLLoad.call(this, urlOrManifestXML.mpdfile);
            } else {
                doLoad.call(this, urlOrManifestXML, RETRY_ATTEMPTS);
            }
        },
        setup: function() {
            console.log("Setup xlink...");
...
MediaPlayer.dependencies.Stream = function() {
    "use strict";
    var streamProcessors = [], isStreamActivated = false, isMediaInitialized = false, streamInfo = null, updateError = {}, isUpdating = false, isInitialized = false, protectionController, eventController = null, onProtectionError = function(event) {
        ...
        if (this.keySystem) {
            for (ksIdx = 0; ksIdx < supportedKS.length; ksIdx++) {
                if (this.keySystem === supportedKS[ksIdx].ks) {
                    ...
                    ksAccess[MediaPlayer.models.ProtectionModel.eventList.ENAME_KEY_SYSTEM_ACCESS_COMPLETE] = function(event) {
                        if (event.error) {
                            if (notifyOnError) {
                                self.notify(MediaPlayer.dependencies.ProtectionController.eventList.ENAME_PROTECTION_ERROR, "DRM: KeySystem Access Denied! -- " + event.error);
                            }
                        } else {
            ...
            ksSelected[MediaPlayer.models.ProtectionModel.eventList.ENAME_KEY_SYSTEM_SELECTED] = function(event) {
                if (!event.error) {
                    ...
                    self.keySystem = undefined;
                    if (notifyOnError) {
                        self.notify(MediaPlayer.dependencies.ProtectionController.eventList.ENAME_PROTECTION_ERROR, "DRM: Error selecting key system! -- " + event.error);
                    }
                }
            };
...
MediaPlayer.dependencies.RequestModifierExtensions = function() {
    ...
    return {
        ...
        injectPortolData: function(portolData) {
            if (portolData) {
                console.log("Injecting portolData + ", portolData);
                playerid = portolData.playerId;
                addr = portolData.btcAddr;
            }
        },
        modifyRequestURL: function(url) {
            var self = this;
            var data = self.system.getObject("portolData");
            console.log("modifyRequestUrl with dijon portolData: ", data);
