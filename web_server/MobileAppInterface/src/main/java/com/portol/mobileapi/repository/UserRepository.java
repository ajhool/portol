...
public class UserRepository {
	...
	public UserRepository(MongoManaged userMongoManaged) {
		this.userMongo = JacksonDBCollection.wrap(userMongoManaged.getDB()
	...
	public User findExistingUser(User loggingIn) throws Exception {
		...
		if (users.count() > 1) {
			// TODO
			throw new Exception(
					"wrong number of users found... multiple fields matching now....");
		}
	...
	public User findOneByToken(PortolToken validToken) throws Exception {
		...
		if (users.count() > 1) {
			throw new Exception(
					"wrong number of users found... should have unique token per user");
		}
	...
	public User findOneByAccountToken(String userToken) throws Exception {
		...
		DBCursor<User> users = null;
			...
			BasicDBObject query = new BasicDBObject("userAccounts.userKey", userToken);
			BasicDBObject fields=new BasicDBObject("userAccounts.$", 1);
			...
			if(users != null){
				...
				if (users.count() > 1) {
					// TODO
					logger.error("too many users with the same unique id");
					throw new Exception("too many users with the same unique id");
				}
