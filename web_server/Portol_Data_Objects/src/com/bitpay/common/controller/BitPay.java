...
import com.bitpay.common.model.Rate;
import com.bitpay.common.model.Rates;
import com.bitpay.common.model.Token;
...
public class BitPay {
	...
	private static final String BITPAY_URL = "https://test.bitpay.com/";
	...
	private String APILimiter_filename = "bitpaylim.dat";
	public static final int API_LIMIT = 1000;

	/**
	 * Constructor for use if the keys and SIN are managed by this library.
	 * 
	 * @param clientName
	 *            - The label for this client.
	 * @param envUrl
	 *            - The target server URL.
	 * @throws Exception
	 */
	public BitPay(String clientName, String envUrl) throws Exception {
	...
	public BitPay(String clientName) throws Exception {
		this(clientName, BITPAY_URL);
	}

	public BitPay() throws Exception {
		this(BITPAY_PLUGIN_INFO, BITPAY_URL);
	}

	/**
	 * Constructor for use if the keys and SIN were derived external to this
	 * library.
	 * 
	 * @param ecKey
	 *            - An elliptical curve key.
	 * @param clientName
	 *            - The label for this client.
	 * @param envUrl
	 *            - The target server URL.
	 * @throws Exception
	 */
	public BitPay(ECKey ecKey, String clientName, String envUrl) throws Exception {
		_ecKey = ecKey;
		openDB();
		this.deriveIdentity();
	...
	public BitPay(ECKey ecKey, String clientName) throws Exception {
		this(ecKey, clientName, BITPAY_URL);
	}

	public BitPay(ECKey ecKey) throws Exception {
	...
	private boolean openDB() {
		limitDB = DBMaker.newFileDB(new File(APILimiter_filename)).closeOnJvmShutdown().make();

		logger.info("value of atomic int upon opening: " + limitDB.getAtomicInteger("bitpayLimit").get());

		return true;
	}

	private static Logger logger = LoggerFactory.getLogger(BitPay.class);

	public synchronized boolean checkAPILimit() throws Exception {

		if (limitDB == null || limitDB.isClosed()) {
			this.openDB();
		}

		Atomic.Integer currentCount = limitDB.getAtomicInteger("bitpayLimit");

		if (currentCount.get() > API_LIMIT) {
			throw new Exception("exceeded self-enforced API limit of: " + API_LIMIT);

		} else {
			currentCount.incrementAndGet();
			logger.info("Incremented api call count to: " + currentCount.get());
			limitDB.commit();
		}
	...
	public void authorizeClient(String pairingCode) throws Exception {
		...
		try {
			...
			throw new BitPayException(
					"Error - failed to deserialize BitPay server response (Tokens) : " + e.getMessage());
		} catch (IOException e) {
			throw new BitPayException(
					"Error - failed to deserialize BitPay server response (Tokens) : " + e.getMessage());
		}
	...
	public String requestClientAuthorization(String facade) throws Exception {
		Token token = new Token();
		token.setId(_identity);
		token.setGuid(this.getGuid());
		token.setNonce(getNextNonce());
		...
		try {
			...
			if (tokens.size() != 1) {
				throw new BitPayException(
						"Error - failed to get token resource; expected 1 token, got " + tokens.size());
			}
	...
	public Invoice createInvoice(Invoice invoice, String facade) throws Exception {
		...
		try {
			...
			throw new BitPayException(
					"Error - failed to deserialize BitPay server response (Invoice) : " + e.getMessage());
		} catch (IOException e) {
			throw new BitPayException(
					"Error - failed to deserialize BitPay server response (Invoice) : " + e.getMessage());
		}
	...
	public List<Invoice> getInvoices(String dateStart, String dateEnd) throws Exception {
		...
		try {
			invoices = Arrays
					.asList(new ObjectMapper().readValue(this.responseToJsonString(response), Invoice[].class));
		} catch (JsonProcessingException e) {
			throw new BitPayException(
					"Error - failed to deserialize BitPay server response (Invoices) : " + e.getMessage());
		} catch (IOException e) {
			throw new BitPayException(
					"Error - failed to deserialize BitPay server response (Invoices) : " + e.getMessage());
		}
	...
	public Rates getRates() throws Exception {
		...
		try {
			...
			throw new BitPayException(
					"Error - failed to deserialize BitPay server response (Rates) : " + e.getMessage());
		} catch (IOException e) {
			throw new BitPayException(
					"Error - failed to deserialize BitPay server response (Rates) : " + e.getMessage());
		}
	...
	private void initKeys() throws IOException {
		if (KeyUtils.privateKeyExists()) {
			...
			// _ecKey =
			// KeyUtils.createEcKeyFromHexStringFile("C:\\Users\\key.priv");
	...
	private Hashtable<String, String> responseToTokenCache(HttpResponse response) throws BitPayException {
		...
		try {
			...
			throw new BitPayException(
					"Error - failed to deserialize BitPay server response (Token array) : " + e.getMessage());
		} catch (IOException e) {
			throw new BitPayException(
					"Error - failed to deserialize BitPay server response (Token array) : " + e.getMessage());
		}
	...
	private boolean tryGetAccessTokens() throws Exception {
	...
	private int getAccessTokens() throws Exception {
		this.clearAccessTokenCache();
	...
	private HttpResponse get(String uri, Hashtable<String, String> parameters) throws Exception {
	...
	private HttpResponse get(String uri) throws Exception {
		return this.get(uri, null);
	}

	private HttpResponse post(String uri, String json, boolean signatureRequired) throws Exception {
	...
	private HttpResponse post(String uri, String json) throws Exception {

		return this.post(uri, json, false);
	}

	private HttpResponse postWithSignature(String uri, String json) throws Exception {
