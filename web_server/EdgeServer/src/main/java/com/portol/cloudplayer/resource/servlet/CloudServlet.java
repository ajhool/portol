package com.portol.cloudplayer.resource.servlet;
...
public class CloudServlet extends HttpServlet {
	...
	public static final Logger logger = LoggerFactory.getLogger(CloudServlet.class);
	...
	private static class CachedAsset {
		...
		private CachedAsset(byte[] resource, long lastModifiedTime, String fileName, String playerId) {
			this.resource = resource;
			this.playerId = playerId;
			this.fileName = fileName;
			this.eTag = '"' + Hashing.murmur3_128().hashBytes(resource).toString() + playerId + '"';
	...
	/**
	 * Creates a new {@code CloudServlet} that serves files loaded from
	 ...
	 * @param returnSvc
	 */
	public CloudServlet(String resourcePath, String uriPath, String indexFile, Charset defaultCharset, Client jClient,
			LocalPlayerRepository localPlayers, EdgeInstance _this, ReturnToQService returnSvc) {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		final String btcAddr = req.getParameter("addr");
		// level 1 verification

		boolean requestValid = verifyRequest(playerId, btcAddr, req);
		...
		Path path = Paths.get(req.getPathInfo());

		boolean eligible;
		try {
			eligible = this.verifyEligibility(playerId, path.getFileName().toString(), req);
		} catch (PreviewException e) {
			resp.sendError(503, "preview unavailable");
			return;
		}

		if (!eligible) {
			logger.info("player: " + playerId + " is ineligible to get file: " + path.getFileName().toString());
			resp.sendError(401, "ineligible to play in preview mode");
			return;
		}
		try {
			...
			final CachedAsset cachedAsset = loadAsset(builder.toString(), playerId);
			...
			// output.close();
			// spin off updater thread for the player state

			Thread updater = new Thread() {
				@Override
				public void run() {
					synchronized (mutex) {
						String playerToUpdate = playerId;
						int latest = parseSeqNoFromFilename(cachedAsset.getFileName());

						Player toUpdate = localPlayers.findOneById(playerToUpdate);

						com.portol.common.model.StreamState mostRecent = toUpdate.getsState();

						if (mostRecent == null) {
							mostRecent = new StreamState(0, 0, 0);
						}

						if (mostRecent.getMaxSent() < latest) {
							mostRecent.setMaxSent(latest);
							toUpdate.setsState(mostRecent);
							localPlayers.save(toUpdate);
						}
					}
				}

				private int parseSeqNoFromFilename(String fileName) {
					...
					try {
						...
						logger.info("defaulting to seq number 0 for filename: " + fileName);
						result = 0;
					}
	...
	private boolean verifyRequest(String playerId, String btcAddr, HttpServletRequest req) {
		Player matching = this.localPlayers.findOneById(playerId);

		if (matching == null || matching.getBtcAddress() == null) {
			return false;
		}

		if (matching.getBtcAddress().equalsIgnoreCase(btcAddr)) {
			return true;
		} else
			return false;
	...
	private boolean verifyEligibility(String playerId, String fileName, HttpServletRequest req)
			throws PreviewException {
		Player matching = this.localPlayers.findOneById(playerId);

		if (matching.getPlayerPayment().getStatus() == Status.COMPLETE) {
			return true;
		}
		...
		String seqNo = fileName.substring(delimiter + 1, fileName.lastIndexOf("."));

		String repId = fileName.substring(0, delimiter);

		int wantedNum = Integer.parseInt(seqNo);
		int maxAllowed;
		try {
			maxAllowed = _this.getServing().getPreviewSegments().get(Content.stripPeriods(repId));
		} catch (Exception e) {
			throw new PreviewException("preview not available for this content");
		}

		if (maxAllowed < wantedNum) {
			return false;
		} else
			return true;
	...
	private CachedAsset loadAsset(String key, String idOfRequester) throws URISyntaxException, IOException {
		...
		if (ResourceURL.isDirectory(requestedResourceURL)) {
			if (indexFile != null && !key.contains("mp4") && !key.contains("m4s")) {
				...
				// return null;
				logger.warn("possbile auth leak exploited");
			}
		}
		...
		// update player object if possible
		String filename = toRead.getName();

		if (idOfRequester == null) {
			idOfRequester = "";
		}
		return new CachedAsset(Files.readAllBytes(toRead.toPath()), lastModified, filename, idOfRequester);
	}
