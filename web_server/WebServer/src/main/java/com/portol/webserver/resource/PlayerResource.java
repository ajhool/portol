...
public class PlayerResource {
	...
	public PlayerResource(File studioRoot, Charset defaultCharset) {
		super();
	...
	private CachedAsset loadAsset(String key) throws URISyntaxException, IOException {
		...
		final String absoluteRequestedResourcePath = studioRoot.getAbsolutePath() + "/" + key;
	...
	public void serveIndex(@Auth(required = true) BasicUser user, @Context HttpServletRequest req) throws Exception {
		...
		try {
			...
			final CachedAsset cachedAsset = loadAsset(reqPath.toString());
			if (cachedAsset == null) {
